...
class PressMotionNode(Node):
    ...
    SURFACE_Z = 0.20 # where the surface Z exe is located
    PRESS_Z = 0.05
    PRECONTACT_Z = 0.35
    FORCE_LIMIT_N = 30.0
    ...
    def __init__(self):
        ...
        self._action_client = ActionClient(self, MoveGroup, "/lbr/move_action") # ROS2 client

        self.get_logger().info("waiting move_group...")
        self._action_client.wait_for_server()
        self.get_logger().info("move_group [OK].")

        self._discover_ft_topic()
        if self._active_ft_topic is None:
            self.get_logger().error(
                "there weren't any FT sensor active"
                "check if FT is a publisher"
            )
            return
    ...
    def _discover_ft_topic(self, timeout_per_topic: float = 2.0):
        self.get_logger().info("searching FT topic in activity")
        ...
        for topic in FT_TOPIC_CANDIDATES:
            ...
            if found.is_set():
                self.get_logger().info(f"  -> active FT topic: {topic}")
        ...
        self.get_logger().warning("No FT topics were publishing data")
    ...
    def _run_ft_diagnostic(self, duration: float = 1.0):
        self.get_logger().info(
            f"FT diagnostic ({duration:.0f} s) — robot on waiting position"
        )
        t0 = time.time()
        samples = 0
        ...
        if self._first_msg_time and self._msg_count > 0:
            ...
            self.get_logger().info(
                f"  estimated frequenci of sensor: {freq:.1f} Hz "
                f"({self._msg_count} msgs in {elapsed:.1f} s)"
            )
        self.get_logger().info("finished FT diagnostic")

    # genera una ventana de tiempo en la que lee las fuerzas en los 3 ejes, si no hay fuerzas devuelve 0.0, 0.0, 0.0
    ...
    def send_goal_and_wait(self, goal: MoveGroup.Goal, label: str) -> bool:
        ...
        if not self._action_client.wait_for_server(timeout_sec=10.0):
            self.get_logger().error("action server no avaliable")
            return False
        ...
        if goal_handle is None or not goal_handle.accepted:
            self.get_logger().error(f"goal denied: {label}")
            return False

        self.get_logger().info("goal accepted, waiting results")
        result_future = goal_handle.get_result_async()
        rclpy.spin_until_future_complete(self, result_future)
        response = result_future.result()

        if response is None:
            self.get_logger().error(f"answer None: {label}")
            return False

        ec = response.result.error_code.val
        codes = {
            1: "SUCCESS (1)",
            -2: "PLANNING_FAILED (-2): no valid way OPML detected no goal",
            -4: "MOTION_PLAN_INVALIDATED_BY_ENVIRONMENT_CHANGE (-4): detected colision through OMPL",
            -5: "CONTROL_FAILED (-5): controller could not follow trajectory",
            -7: "TIMED_OUT (-7): exces on waiting time",
            -9: "START_STATE_IN_COLLISION (-9): El brazo ya colisiona con algo antes de iniciar movimiento.",
            -12: "GOAL_IN_COLLISION (-12): final destiny goes through an object",
        }
        ...
        if ec == 1:
            ...
            return True
        elif ec in (-4, -5, -12) and label == "pressing object or surface":
            self.get_logger().info(
                f"  {label}: {code_str}\n"
                f"    -> NOTA: RViz/MoveIt detects a collision\n"
                f"    -> all good is a prevesible move"
            )
            return True
        else:
            self.get_logger().error(f"  {label} declined by: {code_str}")
            return False

    def run_press_sequence(self):
        self.get_logger().info("--- transport")
        joint_names = [
            "lbr_A1",
            "lbr_A2",
            "lbr_A3",
        ...
        if not self.send_goal_and_wait(goal_transport, "Transport pose"):
            self.get_logger().warning("the initial transport pose could not be achieved")

        num_ciclos = 3

        for ciclo in range(1, num_ciclos + 1):
            self.get_logger().info(f"--- starting cicles {ciclo} of {num_ciclos}")

            self.get_logger().info(f"--- cicle {ciclo}")
            ...
            if not self.send_goal_and_wait(goal_pre, "pre-contact"):
                self.get_logger().error("denied: could not do pre-contact correctly")
                return

            self._log_force("pre-contact")
            self.get_logger().info("waiting 1.00s for stability")
            time.sleep(1.0)
            ...
            if not self.send_goal_and_wait(goal_press, "pressing"):
                self.get_logger().warning(
                    "pressing failed by real contact"
                )
            ...
            while time.time() - t0 < 5.0:
                ...
                if abs(fz) > self.FORCE_LIMIT_N or mag > self.FORCE_LIMIT_N * 1.2:
                    self.get_logger().warning(
                        f"threshold exceeded (|Fz|={abs(fz):.1f}N, |F|={mag:.1f}N > "
                        f"{self.FORCE_LIMIT_N}N"
                    )
                    force_exceeded = True
                    break
                time.sleep(0.15)
            ...
            if not force_exceeded:
                self._log_force("final pressing")
            ...
            self.send_goal_and_wait(goal_ret, "return")

        self.get_logger().info("finished cicles")
