...
class MoveGroupPythonIntefaceControl(Node):
    ...
    def _plan_and_execute(
        self, trajectory, success: bool = True, allow_collision: bool = False
    ) -> bool:
        """
        Publica el plan en RViz2 y lo ejecuta via move_group (con monitoreo
        de colisiones).

        Si allow_collision=True, acepta codigos de colision (ej: penetracion
        de esponja). La planificacion ya fue hecha con avoid_collisions=False
        para esos casos.
        """
        ...
        # --- Ejecutar via move_group execute_trajectory (monitoreo) ---
        try:
            ...
            if not hasattr(self, "_exec_client"):
                self._exec_client = ActionClient(
                    ...
                    "/lbr/execute_trajectory",
                )

            if not self._exec_client.wait_for_server(timeout_sec=5.0):
                self.get_logger().warn(
                    "execute_trajectory action not available; "
                    "falling back to pymoveit2.execute()"
                )
                jt = (
                ...
                self._moveit2.wait_until_executed()
                self.get_logger().info("Trajectory executed (fallback path).")
                return True
            ...
            self.get_logger().info(
                ...
                f"({len(jt.points)} pts, "
                f"allow_collision={allow_collision})"
            )
            ...
            if response is None:
                self.get_logger().error(
                    "Null response from ExecuteTrajectory."
                )
                return False
            ...
            codes = {
                1: "SUCCESS",
                -4: "MOTION_PLAN_INVALIDATED_BY_ENVIRONMENT_CHANGE (collision detected during execution)",
                -5: "CONTROL_FAILED",
                -9: "START_STATE_IN_COLLISION",
                -12: "GOAL_IN_COLLISION",
            }
            code_str = codes.get(ec, f"CODE_{ec}")

            if ec == 1:
                self.get_logger().info(
                    f"Trajectory executed successfully. [{code_str}]"
                )
                return True
            elif ec in (-4, -12) and allow_collision:
                # Colision esperada (penetracion de esponja): aceptar
                self.get_logger().info(
                    f"MoveGroup reports {code_str} — collision is expected "
                    f"(penetration mode). Accepting result."
                )
                return True
            elif ec in (-4, -9, -12):
                # Colision NO esperada: abortar por seguridad
                self.get_logger().error(
                    f"UNEXPECTED COLLISION detected [{code_str}]. "
                    f"Stopping for safety."
                )
                return False
            else:
                self.get_logger().error(
                    f"Execution failed with error code: {code_str}"
                )
                return False
    ...
    def follow_trajectory_speed(
        ...
        Si allow_collision=True, desactiva el chequeo de colisiones en la
        planificacion cartesiana y acepta codigos de colision en ejecucion
        (ej: penetracion de esponja).
        """
        if ang_speeds is None:
            ang_speeds = []
        try:
            trajectory, success = self.compute_cartesian_path_velocity_control(
                waypoints, speeds, EE_ang_speed=ang_speeds,
                max_linear_accel=accel, avoid_collisions=not allow_collision,
            )
        except Exception as exc:
        ...
        executed = self._plan_and_execute(
            trajectory, success, allow_collision=allow_collision
        )
        time.sleep(0.5)
        return executed
    ...
    def go_to_pose_speed(
        ...
        try:
            ...
            trajectory, success = self.compute_cartesian_path_velocity_control(
                waypoints, [speed], EE_ang_speed=ang_speed,
                max_linear_accel=accel, avoid_collisions=not allow_collision,
            )
        except Exception as exc:
    ...
    def _plan_cartesian_segment(
        self, waypoints: list, step: float = 0.002,
        start_joint_positions: list = None,
        start_joint_names: list = None,
        avoid_collisions: bool = True,
    ) -> RobotTrajectory | None:
        """
        Planifica un segmento cartesiano llamando directamente al servicio
        GetCartesianPath.

        Si avoid_collisions=True, activa el chequeo de colisiones durante la
        planificacion (recomendado para movimientos seguros).
        Para penetracion intencional de objetos, usar avoid_collisions=False.

        Si se pasan start_joint_positions/start_joint_names, se usan como
        ...
        if not hasattr(self, "_cartesian_client"):
            self._cartesian_client = self.create_client(
                GetCartesianPath, "/lbr/compute_cartesian_path"
            )
        ...
        # Estado inicial: usar el proporcionado o el actual del robot
        if start_joint_positions is not None and start_joint_names is not None:
            joint_names = start_joint_names
            joint_positions = list(start_joint_positions)
        else:
            ...
            if current_js is None:
                self.get_logger().error(
                    "No joint_state for cartesian planning start."
                )
                return None
            joint_names = list(current_js.name)
            joint_positions = list(current_js.position)
        ...
        self.get_logger().info(
            f"GetCartesianPath | waypoints={len(req.waypoints)} | "
            f"max_step={req.max_step:.4f} | "
        ...
        if result is None:
            self.get_logger().error(
                "GetCartesianPath: no response from service."
            )
            return None

        if result.fraction < 0.1:
            self.get_logger().error(
                f"GetCartesianPath: fraction too low "
                f"({result.fraction:.2f}). "
                f"Probable collision or unreachable target."
            )
            return None

        self.get_logger().info(
            f"GetCartesianPath OK | fraction={result.fraction:.3f} | "
            f"pts={len(result.solution.joint_trajectory.points)}"
        )
    ...
    def compute_cartesian_path_velocity_control(
        ...
        Usa GetCartesianPath con avoid_collisions para los segmentos y
        compute_fk para la cinematica directa.
        """
        ...
        # ── 1. Planificar segmentos cartesianos via GetCartesianPath ──
        all_plans = []
        joint_names = None
        ...
        # Estado inicial para el primer segmento
        start_positions = list(current_js.position)
        start_names = list(current_js.name)

        for traj in waypoints_list:
            # traj es una lista de Poses (waypoints); usa GetCartesianPath
            # con avoid_collisions=True para chequear colisiones
            trajectory = self._plan_cartesian_segment(
                waypoints=traj,
                step=step,
            ...
            if trajectory is None:
                self.get_logger().error("Cartesian segment plan returned None.")
                return None, False

            # trajectory ya es RobotTrajectory (desde GetCartesianPath)
            rt = trajectory if isinstance(trajectory, RobotTrajectory) else RobotTrajectory()
            if not isinstance(trajectory, RobotTrajectory):
                rt.joint_trajectory = trajectory
            all_plans.append(rt)

            if joint_names is None and hasattr(rt, "joint_trajectory"):
                joint_names = list(rt.joint_trajectory.joint_names)

            # Encadenar: el inicio del siguiente segmento es el final de este
            last_pt = rt.joint_trajectory.points[-1]
            start_positions = list(last_pt.positions)
            if joint_names:
                start_names = list(joint_names)
...
def main(args=None):
    ...
    try:
        ...
        for sponge in selected_sponges:
            ...
            # Approach pose con control de velocidad (con chequeo de colisiones)
            target.position.x = x0
            target.position.y = y0
            target.position.z = z0 + offset
            if not control.go_to_pose_speed(
                target, move_speed, accel=100.0, allow_collision=False
            ):
                control.get_logger().error(
            ...
            for p in penetrations:
                ...
                for s in hysteresis_speeds:
                    ...
                    # allow_collision=True: la penetracion es INTENCIONAL
                    ok = control.go_to_pose_speed(
                        target, s, accel=2000.0, allow_collision=True
                    )
                    if not ok:
                        control.get_logger().error(
